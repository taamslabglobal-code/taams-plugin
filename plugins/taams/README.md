...
```
망고 2026년 3월 수입단가 보여줘
아스파라거스를 수입하는 국내 업체 알려줘
당근 도매경매 낙찰가를 서울가락 기준으로 보여줘
```

여러 도구를 엮어야 하는 분석에는 슬래시 명령을 쓴다.

| 명령 | 하는 일 |
|---|---|
| `/taams:report [품목명]` | 수입단가·수입량·산지 기상을 한 리포트로 |
| `/taams:competitors [품목명]` | 국내 수입사 구도와 집중도, 최근 변화 |
| `/taams:auction [농산물명]` | 도매시장 낙찰단가 — 등급 구성까지 갈라서 |
| `/taams:supplier [품목명]` | 해외 수출사 후보를 국가별로 |

단품 조회(단가·수출사 목록 등)는 명령 없이 그냥 물으면 된다.
